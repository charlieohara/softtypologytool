...
error = False
Confusion = False
freqconfusionmatrix = {'tV': {'tV': 880, 'pV': 92, 'kV': 165, 'V': 175},
                       'pV': {'tV': 16, 'pV': 1264, 'kV': 58, 'V': 328},
                       'kV': {'pV': 116, 'tV': 41, 'kV': 1253, "V": 177},
                       'V': {'pV': 176, 'tV': 7, 'kV': 51, "V": 1425}, 'Vp': {'Vp': 1095, 'Vt': 114, 'Vk': 217},
                       'Vt': {'Vp': 99, "Vt": 1171, 'Vk': 132}, 'Vk': {'Vp': 149, 'Vt': 90, 'Vk': 1202}}
freqconfusionmatrix = {'tV': {'tV': 1082, 'pV': 45, 'kV': 12, 'V': 19}, 'pV': {'tV': 11, 'pV': 1109, 'kV': 17, 'V': 18},
                       'kV': {'pV': 25, 'tV': 17, 'kV': 1108, "V": 19}, 'V': {'V': 1},
                       'Vp': {'Vp': 720, 'Vt': 102, 'Vk': 139, 'V': 72}, 'Vt': {'Vp': 90, "Vt": 830, 'Vk': 65, 'V': 99},
                       'Vk': {'Vp': 35, 'Vt': 48, 'Vk': 1019, "V": 49}}

confusionmatrix = {}
for x in freqconfusionmatrix:
    confusers = freqconfusionmatrix[x]
    confusionmatrix[x] = {}
    for y in confusers:
...
def load():
    ...
    inputorder, tableauname, tableau, start, savedpatterns, learningrate, generations, iterations, runs, bigtyp, gengraphs, itgraphs = pickle.load(
        loadee)
    resetEntries()
    LOADING = TRUE
    importtableau()
...
def runsims():
    ...
    def graphs():
        ...
        def graphtyp():
            f.clf()
            colors = ['b', 'g', 'r', 'c', 'm', 'y', 'k', 'w', 'aqua', 'fuchsia', 'purple', 'navy', 'olive', 'maroon',
                      'lime', ]
        ...
        graphv = StringVar()
        graphv.set("it")
        Radiobutton(graphpopup, text="Within Generation Learning", variable=graphv, value="it",
                    command=buildlangs).grid(column=0, row=0)
        if gen.get() >= 1:
            Radiobutton(graphpopup, text="Generational Change", variable=graphv, value="gen", command=buildlangs).grid(
                column=1, row=0)
        Radiobutton(graphpopup, text="Result Typology", variable=graphv, value="typ", command=buildlangs).grid(column=2,
                                                                                                               row=0)
    ...
    # Test to see if any patterns are being searched over, throw an error window if not
        ...
        if LOADING == FALSE:
            for l in languages:
                    ...
                    for g in range(generations):
                        ...
                        for i in range(iterations):
                            ...
                            if error:
                                ...
                                if rand < .4 and Confusion:
                                    confusionprobs = confusionmatrix[observedoutput]
                                    newoutput = np.random.choice(confusionprobs.keys(), 1, p=confusionprobs.values())[0]
                                    if newoutput != observedoutput:
                                        if newoutput == "V":
                                            observedoutput = newoutput
                                        elif newoutput == input:
                                            observedoutput = newoutput
                                        else:
                                            input = newoutput
                                            observedoutput = newoutput
                                    ...
                                    if rand < errorchance:
                                        ...
                                        if switch:
                                            observedoutput1 = \
                                                np.random.choice(
                                                    ([k for k in tableau[input].keys() if k != observedoutput]), 1)[0]
                            ...
                            if (np.array(np.where(w1 < 0)).size > 0):
                                for toolowindex in np.where(w1 < 0):
                                    w1[toolowindex] = w[toolowindex]
                                    # w1[np.where(w1 < 0)] = .1
                                    #
                ...
                firstgarray = map(lambda x: np.reshape(x, newshape=(len(x) / len(inputorder), len(inputorder))),
                                  firstgenbyrun)
                genlistarray = map(
                    lambda y: map(lambda x: np.reshape(x, newshape=(len(x) / len(inputorder), len(inputorder))), y),
                    genbyrun)
